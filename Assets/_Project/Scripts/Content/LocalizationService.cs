...
namespace AetherArk.Content
{
    public sealed partial class LocalizationService
    {
        ...
        private void AddStrings()
        {
            ...
            Add("ui.port_market_tab", "장비 시장", "EQUIPMENT MARKET");
            Add("ui.port_crew_tab", "승무원", "CREW");
            Add("ui.crew_roster", "현역 승무원", "ACTIVE CREW");
            Add("ui.crew_capacity", "정원 {0}", "Capacity {0}");
            Add("ui.port_recruit", "항구 모집 후보", "PORT RECRUIT");
            Add("ui.recruit", "영입", "Recruit");
            Add("ui.level", "숙련 {0}", "Level {0}");
            Add("ui.experience", "경험 {0}", "XP {0}");
            Add("ui.max_level", "최대 숙련", "MAX LEVEL");
            Add("ui.no_recruits", "모집 가능한 후보가 없습니다.", "No recruits are available.");
            Add("crew.recruit_aela", "아엘라", "Aela");
            Add("crew.recruit_durgan", "두르간", "Durgan");
            Add("crew.recruit_kael", "카엘", "Kael");
            Add("crew.recruit_nym", "님", "Nym");
            Add("crew.recruit_rix", "릭스", "Rix");
            Add("crew.recruit_varka", "바르카", "Varka");
            Add("command.recruit_unknown", "알 수 없는 모집 후보입니다.", "Unknown recruit.");
            Add("command.recruit_unavailable", "이 후보는 현재 항구에서 모집할 수 없습니다.", "That candidate is not available at this port.");
            Add("command.recruit_joined", "이미 선단에 합류한 승무원입니다.", "That crew member has already joined.");
            Add("command.crew_capacity", "승무원 정원이 가득 찼습니다.", "Crew capacity is full.");
            Add("command.recruit_cost", "영입에 필요한 인양물이 부족합니다.", "Not enough salvage to recruit.");
            Add("command.crew_recruited", "새 승무원이 합류했습니다.", "A new crew member joined.");
            Add("log.crew_recruited", "{0} 합류.", "{0} joined the crew.");
            Add("log.crew_level_up", "{0} 숙련 상승.", "{0} gained a skill level.");
            ...
            Add("repro.help", "같은 전투를 비교하려면 스냅샷을 저장하고 다시 불러오세요. 일반 저장은 바뀌지 않습니다.", "Capture and reload a battle to compare decisions. Normal saves remain untouched.");
            Add("repro.invalid_seed", "시드는 -2147483648부터 2147483647까지의 정수여야 합니다.", "Seed must be an integer from -2147483648 to 2147483647.");
            Add("repro.invalid_config", "기함 또는 난이도 설정을 확인해 주세요.", "Check the flagship and difficulty settings.");
            Add("repro.invalid_arguments", "재현 실행 옵션을 함께 사용할 수 없습니다. 시드 또는 스냅샷 진입 중 하나만 선택하세요.", "Conflicting reproduction arguments. Choose one seed or snapshot entry point.");
            Add("repro.combat_only", "전투 중인 상태만 스냅샷으로 저장·불러올 수 있습니다.", "Only active combat states can be captured or loaded as snapshots.");
            Add("repro.started", "격리된 시드 테스트를 시작했습니다.", "Started an isolated seeded test.");
            Add("repro.saved", "새 전투 스냅샷을 저장했습니다. 이전 파일은 보존됩니다.", "Captured a new battle snapshot. Earlier files are preserved.");
            Add("repro.loaded", "전투 상태를 복원했습니다. 일시정지 상태로 시작합니다.", "Restored the battle state. It opens paused.");
            Add("repro.different_build", "스냅샷의 시뮬레이션/Unity 빌드가 다릅니다. 상태는 복원했지만 이후 결과 일치는 보장하지 않습니다.", "Snapshot uses a different simulation/Unity build. State was restored, but subsequent outcomes may differ.");
            Add("repro.snapshot_missing", "스냅샷 파일이 없습니다. 전체 경로를 확인하거나 먼저 전투를 저장하세요.", "Snapshot not found. Check the full path or capture a battle first.");
            Add("repro.invalid_snapshot", "유효한 전투 스냅샷이 아니거나 파일이 손상되었습니다. 현재 게임은 유지됩니다.", "Invalid or damaged battle snapshot. The current game is unchanged.");
            Add("repro.unsupported_version", "지원하지 않는 스냅샷/세이브 버전입니다. 현재 게임은 유지됩니다.", "Unsupported snapshot/save version. The current game is unchanged.");
            Add("repro.file_error", "파일을 읽거나 저장하지 못했습니다. 경로·권한·남은 공간을 확인하세요.", "Could not read or save the file. Check its path, permissions and free space.");
            Add("repro.selected", "최근 스냅샷 경로를 선택했습니다. 불러오기를 누르면 복원합니다.", "Selected the latest capture path. Press Load to restore it.");
            Add("repro.returned", "원래 게임으로 돌아왔습니다. 이 화면을 닫으면 기존 상태로 계속합니다.", "Returned to your normal game. Close this panel to continue from its original state.");
            ...
            Add("enum.crewrole.captain", "함장", "Captain");
            Add("enum.crewrole.resonator", "공명사", "Resonator");
            Add("enum.crewrole.engineer", "기관사", "Engineer");
            Add("enum.crewrole.gunner", "포술사", "Gunner");
            Add("enum.crewrole.pilot", "조종사", "Pilot");
            Add("enum.crewrole.medic", "의무관", "Medic");
            Add("enum.crewrole.marine", "해병", "Marine");
            Add("enum.crewrole.navigator", "항법사", "Navigator");
            Add("trait.steadfast", "불굴", "Steadfast");
            Add("trait.steadfast.desc", "화재·침입 피해 10% 감소, 구조 대기시간 +2초", "-10% fire/intruder damage and +2s rescue window");
            Add("trait.attuned", "공명 감응", "Attuned");
            Add("trait.attuned.desc", "과부하 불안정도 15% 감소, 사고율 20% 감소", "-15% overload instability and -20% accident risk");
            Add("trait.fireproof", "내화 체질", "Fireproof");
            Add("trait.fireproof.desc", "화재 피해 65% 감소", "-65% fire damage");
            Add("trait.quick_hands", "빠른 손", "Quick Hands");
            Add("trait.quick_hands.desc", "수리 속도 12% 증가", "+12% repair speed");
            Add("trait.light_step", "가벼운 발걸음", "Light Step");
            Add("trait.light_step.desc", "저산소 피해 30% 감소", "-30% low-oxygen damage");
            Add("trait.rescuer", "구조 전문가", "Rescuer");
            Add("trait.rescuer.desc", "같은 구획의 쓰러진 승무원 사망 시간 50% 지연", "Downed crew in the same room bleed out 50% slower");
            Add("background.exile", "망명 지휘관", "Exiled Officer");
            Add("background.exile.desc", "제압력 8% 증가, 화재·침입 피해 5% 감소", "+8% boarding power and -5% fire/intruder damage");
            Add("background.weather_scholar", "기상 학자", "Weather Scholar");
            Add("background.weather_scholar.desc", "기상으로 받는 승무원 피해 25% 감소", "-25% crew damage from weather");
            Add("background.dockwright", "조선공", "Dockwright");
            Add("background.dockwright.desc", "수리 속도 12% 증가", "+12% repair speed");
            Add("background.deck_runner", "갑판 주자", "Deck Runner");
            Add("background.deck_runner.desc", "편대 손실률 10% 감소", "-10% sortie loss");
            Add("background.cloud_medic", "구름바다 의무관", "Cloud Medic");
            Add("background.cloud_medic.desc", "같은 구획 구조 지원, 의무실 치료량 25% 증가", "Aids rescues in the same room and grants +25% infirmary healing");
            Add("background.border_guard", "변경 수비대", "Border Guard");
            Add("background.border_guard.desc", "제압력 15% 증가, 침입자 피해 10% 감소", "+15% boarding power and -10% intruder damage");
