...
Progress toward 1.0 (2026-09-04): 30 flagship modules, the between-region port shop, 18 mounted weapons with hardpoints, three flagships with progress unlocks, nine air wings, twelve enemy silhouettes with thirty region-gated configs, transparent top-down art for those families and the Gate Warden, twenty shared module/weapon/wing role icons, six regions, the Gate Warden finale, one hundred events with region tags, and six mechanically distinct lineages are in. Remaining work is production depth, balance, presentation, accessibility and platform QA rather than another headline system family.
...
Traits and professional backgrounds add smaller, stackable identity modifiers without changing the save schema. The initial six crew cover six traits (`steadfast`, `attuned`, `fireproof`, `quick_hands`, `light_step`, `rescuer`) and six backgrounds (`exile`, `weather_scholar`, `dockwright`, `deck_runner`, `cloud_medic`, `border_guard`). Their effects use the same repair, hazard, boarding, resonance, sortie and healing paths as lineage rules. A rescuer or cloud medic in the same room slows a downed crew member's bleedout; a cloud medic actively posted in the infirmary improves its healing. Missing keys are deliberately neutral so older saves remain playable.

Crew growth is contained within the current run and never becomes permanent numeric metaprogression. Each survivor receives one experience point after a battle; skill level 2 requires four XP and level 3 requires eight more. Higher skill improves general repair and boarding contribution plus role-specific gunnery, navigation, medicine, resonance and sortie handling. Between-region ports expose a separate crew tab with the living roster and one seed-deterministic recruit. A recruit costs salvage, retains an authored lineage/role/trait/background identity, and only one may join per port. The living roster cap is eight; dead crew do not consume an active berth, but their records remain in the run for loss history and deterministic offer exclusion.
